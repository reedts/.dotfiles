...
cavpn() {
	...
	config="/etc/openvpn/btguard.ca.conf"
	if [ -f $pidfile ]; then
		sudo killall $vpnprocess
		sudo rm -f $pidfile
	else
		sudo openvpn --writepid $pidfile --config $config
	fi
}

# Function for mounting the windows and secondary linux data partitions under /mnt
mount_both() {
	sudo mkdir /mnt/Win
	sudo mkdir /mnt/Daten
	sudo mount /dev/sdb1 /mnt/Daten
	sudo mount -t ntfs-3g -o umask=000 /dev/sdc1 /mnt/Win
}
...
mount_unihome() {
    ...
    sudo mount -t cifs //ivv5net/ddfs/user/j_bigg01 /mnt/unihome -o user=j_bigg01,domain=ivv5net,uid=$userid,gid=$grpid
}

# Assemble a programm in assembler
assemble() {
	as -o $1.o --32 -g $1.asm
